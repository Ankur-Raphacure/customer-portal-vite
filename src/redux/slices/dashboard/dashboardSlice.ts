...
export const dashboardSlice = createSlice({
  ...
  reducers: {
    ...
    updateErrorMessage: (state: DashboardState, action: PayloadAction<boolean>) => {
  ...
  extraReducers: (builder: any) => {
    ...
    builder.addCase(getMyBookingsAPI.rejected, (state: DashboardState, action: any) => {
      console.log({ action });
      state.loading = false;
  ...
  builder.addCase(getAllSubscriptions.rejected, (state: DashboardState, action: any) => {
    console.log({ action });
    state.loading = false;
    state.error = action.error?.message ?? "Something went wrong";
  });
  ...
  builder.addCase(getMyPackageDetailsAPI.rejected, (state: DashboardState, action: any) => {
    console.log({ action });
    state.loading = false;
    state.error = action.error?.message ?? "Something went wrong";
  });
